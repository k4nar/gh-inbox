...
<div class="thread">
    <Collapsible.Root open={expanded} onOpenChange={(v) => (expanded = v)}>
        ...
        <Collapsible.Trigger class="thread-header">
            {#if thread.path}
                <svg
                    aria-hidden="true"
                    width="14"
        ...
        {#if !expanded}
            {#if hasNewComments && lastNewComment}
...
.thread :global(.thread-header:hover) {
