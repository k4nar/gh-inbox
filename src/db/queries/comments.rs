...
pub async fn upsert_comment(pool: &SqlitePool, comment: &CommentRow) -> sqlx::Result<()> {
    sqlx::query(
        "INSERT INTO comments (id, pr_id, thread_id, author, body, created_at, comment_type, path, position, in_reply_to_id, html_url)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
pub async fn query_comments_for_pr(pool: &SqlitePool, pr_id: i64) -> sqlx::Result<Vec<CommentRow>> {
    sqlx::query_as::<_, CommentRow>(
        "SELECT id, pr_id, thread_id, author, body, created_at, comment_type, path, position, in_reply_to_id, html_url
         FROM comments
         WHERE pr_id = ?
